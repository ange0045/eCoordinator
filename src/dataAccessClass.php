...
// -- Gets the username for the current user
  ...
  public function getStudents ($fldStudentName){
    ...
    foreach ($stmt as $row)
    {
        $student = new Student ($row['student_id'], $row['student_name'], $row['student_type'], $row['student_email']);
        $students[] = $student;
    }
    return $students;
  }
  ...
  public function updateStudent($id, $s_name, $s_type, $s_email) {
    $sql = "UPDATE student SET student_name = :vName, student_type = :vType, student_email = :vEmail WHERE student_id = :vId";
    $stmt = $this->pdo->prepare($sql);
    $this->pdo->beginTransaction();
    $stmt->execute(['vName'=>$s_name, 'vType'=>$s_type, 'vEmail'=>$s_email, 'vId'=>$id]);
    $this->pdo->commit();
  }
...
public function addStudentCourse($sc_ID, $sc_key, $sc_grade, $sc_comment)
{
  $sql = "INSERT INTO studentcourse VALUES(:fStudentID, :fCourseKey, :fStudentGrade, :fComment)";
  $stmt = $this->pdo->prepare($sql);
  $this->pdo->beginTransaction();
  $stmt->execute(['fStudentID'=>$sc_ID, 'fCourseKey'=>$sc_key, 'fStudentGrade'=>$sc_grade, 'fComment'=>$sc_comment]);
  $this->pdo->commit();
}
